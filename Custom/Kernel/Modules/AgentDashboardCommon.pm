...
# --
# $origin: Znuny - 53cbb8f3f20f3cb1018cfed665b11a45e2841e4a - Kernel/Modules/AgentDashboardCommon.pm
# --
...
#         }
        if ( !$Config->{$Name}->{NeedCustomSearchEngineConnection} ) {
            COLUMNNAME:
            for my $ColumnName (
                qw(Owner Responsible State Queue Priority Type Lock Service SLA CustomerID CustomerUserID)
                )
                ...
                )
            {
                ...
                if ( $ColumnName eq 'CustomerID' || $ColumnName eq 'CustomerUserID' ) {
                    push @{ $ColumnFilter{$ColumnName} }, $FilterValue;
                }
                else {
                    push @{ $ColumnFilter{ $ColumnName . 'ID' } }, $FilterValue;
                }
...
# Znuny-SearchFrontend
# ---
    my $CustomEngineNoConnection;

    if (
        $Object->{Behavior}->{NeedCustomSearchEngineConnection}
        && $Self->{SearchObject}->{Fallback}
    ) {
        $CustomEngineNoConnection = 1;
    }
# ---
...
# Znuny-SearchFrontend
# ---
                CustomEngineNoConnection => $CustomEngineNoConnection,
# ---
            },
        );
...
#        return if !$Backends->{$Name};
        return if !$Backends->{$Name} || $CustomEngineNoConnection;
# ---
    }
